...
```tsx
// creare un area di contenuto scrollabile
// creare 4 bottoni
// - scroll all'inizio
// - scroll alla fine
// - scroll in su della dimensione dell'area di contenuto
// - scroll in giu della dimensione dell'area di contenuto
...
In react è possibile interrompere la propagazione di un errore, in modo da evitare che l'intera applicazione venga compromessa.
Il meccanismo si chiama error boundary, si implementa implementande determinati metodi lifecycle su un componenten basato su classi.
Ad oggi forse l'unico motivo valido per utilizzare un componenten classse e non funzione è proprio per creare un error boundary.
Per far diventare un componente classe un'error boundary, basta implementare anche solo uno dei deu metodi: static getDerivedStateFromError, componentDidCatch
...
class SimpleErrorBoundary extends React.Component<{
  ...
  // this.setState(getDerivedStateFromError(erroreCatturato))
  ...
  // viene schedulato insieme a React.useLayoutEffect
  static getDerivedStateFromError(error: Error) {
    return { hasError: true };
  }
  // serve per eseguire Effetti in caso di errore
  // qui meglio non fare this.setState
  // viene schedulato insieme a React.useEffect
...
// creare un componente error boundary component che permette di riprovare in caso di errore
// SPOILER soluzione
...
React portal è un meccanismo per renderizzare parte dell'applicazione react all'infuori dell'elemento root del dom reale.
Il nome portal è sipirato al fatto teletrasportiamo una prozione dell'applicazione fuori dal suo contenitore.
Da nptare che react portal fa parte del package react-dom e quindi è disponibile solo per applicazioni web.
Le istanze dei componenti nei portal per react si trovano all'interno del componente in cui sono stati dichiarati, (sia a livello di react-dev-tools che per i context).
